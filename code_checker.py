...
class CodeChecker:
        ...
        if cache_key in self.cache:
            ...
            if isinstance(cached_entry, tuple):
                if len(cached_entry) == 3:
                    cached_hash, cached_result, cached_version = cached_entry
                    if cached_hash == file_hash and cached_version == self.cache_version:
    ...
    def _save_to_cache(self, file_path: Path, result: FileResult):
        ...
        cache_key = str(file_path)
        self.cache[cache_key] = (file_hash, result, self.cache_version)
        self._save_cache()
    ...
    def _build_rules(self) -> str:
        ...
        3. EMPTY_UPDATE_METHOD - 继承自MonoBehaviour的类中存在空的Update/LateUpdate/FixedUpdate方法（只有空白/注释/预处理指令才算空）
        ...
        **重要**：EMPTY_UPDATE_METHOD仅在方法体没有任何实际语句时报告（哪怕只有一行有效语句也不违规）

        返回JSON格式：{"violations": [{"line": 1, "line_content": "原始代码内容", "rule": "RULE_NAME", "description": "违规描述"}]}
        如果没有违规，返回：{"violations": []}"""
        return rules_short

    def _classify_editor_condition(self, expression: str) -> str:
        """判断预处理条件是否为Unity编辑器分支"""
        if re.search(r"!\s*UNITY_EDITOR", expression):
            return "non_editor"
        if re.search(r"\bUNITY_EDITOR\b", expression):
            return "editor"
        return "unknown"
    ...
    def _remove_editor_code(self, code: str) -> str:
        ...
        for line in lines:
            ...
            if directive_match:
                ...
                expr = directive_match.group(2).strip()
                current_in_editor = any(kind == "editor" for kind in branch_stack)
                mask_line = current_in_editor

                if directive in ("if", "elif"):
                    branch_kind = self._classify_editor_condition(expr)
                    if branch_kind != "unknown":
                        mask_line = True
                    if directive == "if":
                        branch_stack.append(branch_kind)
                    else:
                        if branch_stack:
                            branch_stack[-1] = branch_kind
                elif directive == "else":
                    if branch_stack and branch_stack[-1] != "unknown":
                        mask_line = True
                    if branch_stack:
                        current = branch_stack[-1]
                        if current == "editor":
                            branch_stack[-1] = "non_editor"
                        elif current == "non_editor":
                            branch_stack[-1] = "editor"
                        else:
                            branch_stack[-1] = "unknown"
                ...
                result_lines.append("" if mask_line else line)
                continue

            if any(kind == "editor" for kind in branch_stack):
                result_lines.append("")
            else:
    ...
    def _prepare_code_for_ai_with_exact_line_numbers(
        ...
        masked_code = self._remove_editor_code(code)
        masked_lines = masked_code.split("\n")

        if len(masked_lines) != len(original_lines):
            print("警告: 编辑器代码处理导致行数变化，回退为原始代码")
            masked_lines = original_lines
        elif len(masked_lines) < len(original_lines):
            masked_lines.extend([""] * (len(original_lines) - len(masked_lines)))
        elif len(masked_lines) > len(original_lines):
            masked_lines = masked_lines[: len(original_lines)]

        prepared_lines = []
        line_mapping: Dict[int, str] = {}

        for i, (orig_line, masked_line) in enumerate(
            zip(original_lines, masked_lines), 1
        ):
            line_mapping[i] = orig_line.rstrip("\n\r")
            prepared_lines.append(f"L{i:4d}: {masked_line.rstrip('\n\r')}")
    ...
    def _strip_comments_from_line(
        self, line: str, in_block_comment: bool
    ) -> Tuple[str, bool]:
        """移除单行中的注释并维护块注释状态"""
        if in_block_comment:
            end_idx = line.find("*/")
    ...
    def _match_update_method_signature(self, line: str) -> Optional[str]:
        """匹配Update/LateUpdate/FixedUpdate方法签名"""
        if not line:
            return None
        match = re.search(r"\bvoid\s+(Update|LateUpdate|FixedUpdate)\s*\(", line)
        if match:
            return match.group(1)
        return None

    def _find_opening_brace(self, lines: List[str], start_index: int) -> Optional[int]:
        """从指定行开始查找方法体起始大括号"""
        in_block_comment = False
        for i in range(start_index, len(lines)):
            processed_line, in_block_comment = self._strip_comments_from_line(
    ...
    def _find_matching_brace(self, lines: List[str], start_index: int) -> Optional[int]:
        """查找与起始大括号匹配的结束行"""
        depth = 0
        found_open = False
        in_block_comment = False
        ...
        for i in range(start_index, len(lines)):
            ...
            processed_line = self._strip_string_literals(processed_line)
    ...
    def _is_method_body_empty(
        self, lines: List[str], start_line: int, end_line: int
    ) -> bool:
        """判断方法体是否为空（忽略空白、注释和预处理指令）"""
        in_block_comment = False
    ...
    def _analyze_update_methods(self, code: str) -> List[Dict[str, Any]]:
        """分析Update相关方法并判断是否为空"""
        ...
        while i <= total_lines:
            ...
            method_name = self._match_update_method_signature(processed_line)
            if method_name:
                open_line = self._find_opening_brace(lines, i)
                if open_line is None:
                    i += 1
                    continue
                close_line = self._find_matching_brace(lines, open_line)
                if close_line is None:
                    i = open_line + 1
                    continue
                is_empty = self._is_method_body_empty(lines, open_line, close_line)
                methods.append(
                    {
                        "name": method_name,
                        "start": open_line,
    ...
    def _find_update_method_for_line(
        self, methods: List[Dict[str, Any]], line_number: int
    ...
    def _parse_api_response(
        ...
        response_text: str,
        line_mapping: Dict[int, str],
        update_method_info: Optional[List[Dict[str, Any]]] = None,
    ) -> List[Violation]:
        """解析API响应，处理行号映射"""
        try:
            update_method_info = update_method_info or []
                    ...
                    if reported_line in line_mapping:
                        ...
                        if extracted_line and extracted_line in line_mapping:
                            ...
                            if extracted_line and extracted_line in line_mapping:
                                ...
                                print(
                                    "警告: 无法确定行号映射 "
                                    f"for line {reported_line} with content '{line_content}'"
                                )

                    check_line = resolved_line if resolved_line is not None else reported_line
                    if rule == "EMPTY_UPDATE_METHOD" and update_method_info:
                        method_info = self._find_update_method_for_line(
                            update_method_info, check_line
                        )
                        if method_info and not method_info["is_empty"]:
                            print(
                                f"跳过非空{method_info['name']}方法: "
                                f"line {check_line}"
                            )
                            continue

                    violation = Violation(
                        line=resolved_line if resolved_line is not None else reported_line,
...
5. EMPTY_UPDATE_METHOD仅在方法体没有任何实际语句时报告（哪怕只有一行有效语句也不违规）
...
{{
    ...
    def check_file(self, file_path: Path) -> FileResult:
        ...
        try:
            ...
            prepared_code, line_mapping = (
                self._prepare_code_for_ai_with_exact_line_numbers(code)
            )
            update_method_info = self._analyze_update_methods(code)
...
5. EMPTY_UPDATE_METHOD仅在方法体没有任何实际语句时报告（哪怕只有一行有效语句也不违规）""",
            ...
            if response.status_code == 200:
                ...
                # 解析响应，使用行号映射
                violations = self._parse_api_response(
                    content, line_mapping, update_method_info
                )
            else:
    ...
    def check_directory(self, dir_path: Path) -> CheckResult:
        ...
        print(
            f"总文件数: {len(cs_files)}, 跳过: {len(skipped_files)}, "
            f"缓存命中: {len(cached_files)}, 需要检查: {len(filtered_files)}"
        )
